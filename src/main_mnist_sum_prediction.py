...
class MNISTSequence(MNIST):
    """MNIST dataset returning sequences of images and their sum as label."""
...
def pad_sequences(batch):
    """Pad sequences in a batch to the same length and create attention masks."""
...
def data_preparation():
    """Prepare train and validation datasets with transforms."""
    ...
    train_set = MNISTSequence(root="data", train=True, download=True, transform=train_transform)
    val_set = MNISTSequence(root="data", train=False, download=True, transform=val_transform)
...
class TransformerModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.self_attention = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(self.embed_dim, nhead=4, dropout=0.0, batch_first=True),
            1,
        )
    ...
    def forward(self, x_seq, attention_mask):
        ...
        if DEBUG:
            print(f"[DEBUG] Input shape: {x_seq.shape}")
        x = x_seq.view(batch * seq, channel, height, width)
        if DEBUG:
            print(f"[DEBUG] Flattened input: {x.shape}")
        z = self.encoder(x)
        if DEBUG:
            print(f"[DEBUG] Encoded: {z.shape}")
        z = z.view(batch, seq, self.embed_dim)
        if DEBUG:
            print(f"[DEBUG] Sequence reshaped: {z.shape}")
        z = self.self_attention(z)
        if DEBUG:
            print(f"[DEBUG] After attention: {z.shape}, mask: {attention_mask.shape}")
        z = z.mean(dim=1)
        if DEBUG:
            print(f"[DEBUG] Mean pooled: {z.shape}")
        z = F.relu(self.fc1(z))
        if DEBUG:
            print(f"[DEBUG] After fc1: {z.shape}")
        z = self.fc2(z)
        if DEBUG:
            print(f"[DEBUG] Output: {z.shape}")
        z = (z + 0.5) * 90  # Rescale output
        return z



class FullyConnectedModel(nn.Module):
    """Fully connected model for fixed-length input sequences."""
    def __init__(self):
        super().__init__()
        self.embed_dim = 128
        self.encoder = ImageEncoder(self.embed_dim)
    ...
    def forward(self, x, attention_mask):
        # attention_mask is not used, for compatibility
        ...
        if DEBUG:
            print(f"[DEBUG] Flattened for FC: {z.shape}")
        z = F.relu(self.fc1(z))
        z = F.relu(self.fc2(z))
        z = self.fc3(z)
        z = (z + 0.5) * 90  # Rescale output
...
class EpochMetricsCallback(L.Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if len(pl_module.training_step_loss_outputs) > 0:
            ...
            train_acc_mean = torch.stack(pl_module.training_step_accuracy_outputs).mean()
...
# ===============================
def plot_metrics(log_df, out_dir):
    """Plot loss, accuracy, and sequence length histograms."""
    # Loss
    train_data = log_df.dropna(subset=["train_loss"])
    val_data = log_df.dropna(subset=["val_loss"])
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    if len(train_data) > 0:
        ax.plot(train_data["epoch"], train_data["train_loss"], label="Training Loss", marker="o")
    if len(val_data) > 0:
        ax.plot(val_data["epoch"], val_data["val_loss"], label="Validation Loss", marker="s")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ...
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    if len(train_data) > 0:
        ax.plot(train_data["epoch"], train_data["train_accuracy"], label="Training Accuracy", marker="o")
    if len(val_data) > 0:
        ax.plot(val_data["epoch"], val_data["val_accuracy"], label="Validation Accuracy", marker="s")
    ax.set_xlabel("Epoch")
...
def run_training_and_prediction():
    ...
    run_name = f"mnist_sum{now}_{MODEL_TYPE}"
